#include <string>
using std::string;
#include <cstring>
using std::strncpy;
#include "ClientData.h"
ClientData::ClientData( int accValue, string lnValue, string fnValue, double balanceValue )
{
setAccNumber( accValue );
setLastName( lnValue );
setFirstName( fnValue );
setBalance( balanceValue );
}
int ClientData::getAccNumber() const
{
return acc;
}
void ClientData::setAccNumber( int accNmbValue )
{
acc = accNmbValue;
}
string ClientData::getLastName() const
{
return lastName;
}
void ClientData::setLastName( string lnString )
{
const char *lnValue = lnString.data();
int length = lnString.size();
length = ( length < 15 ? length : 14 );
strncpy( lastName, lnValue, length );
lastName[ length ] = '\0';
}
string ClientData::getFirstName() const
{
return firstName;
}
void ClientData::setFirstName( string fnString )
{
const char *fnValue = fnString.data();
int length = fnString.size();
length = ( length < 10 ? length : 9 );
strncpy( firstName, fnValue, length );
firstName[ length ] = '\0';
}
double ClientData::getBalance() const
{
return balance;
}
void ClientData::setBalance( double balanceValue )
{
balance = balanceValue;
}
